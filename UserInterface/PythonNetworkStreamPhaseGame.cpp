...
bool CPythonNetworkStream::RecvBiologPacket()
{
	tBiologGCInformation p;
	if (!Recv(sizeof(p), &p))
	{
		Tracenf("Recv tBiologGCAction Packet Error");
		return false;
	}

	PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "Binary_BioAffReset", Py_BuildValue("()"));
	const size_t size = sizeof(p.bAffect) / sizeof(p.bAffect[0]);
	for (size_t i = 0; i < size; ++i)
	{
		const BYTE type = p.bAffect[i];
		const WORD val = p.bAffValue[i];
		PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "Binary_BioAffUpdate",
			Py_BuildValue("(ii)", type, val));
	}

	PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "Binary_BioUpdate",
		Py_BuildValue("(iiiiiii)", p.dwItemVnum, p.dwSoulVnum, p.bGivenCount, p.bRequiredCount, p.bChance, p.iTime, p.bState));
