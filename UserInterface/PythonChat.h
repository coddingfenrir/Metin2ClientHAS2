...
class CWhisper
{
	public:
		typedef struct SChatLine
		{
			...
			CGraphicExpandedImageInstance* pEmpireFlagImageInstance{};
...
#if defined(__BL_MULTI_LANGUAGE_PREMIUM__)
		void AppendChat(int iType, const char* c_szChat, BYTE bEmpire = 0, const std::string& countryName = "");
#else
		void AppendChat(int iType, const char * c_szChat, BYTE bEmpire = 0);
#endif
#else
...
class CPythonChat : public CSingleton<CPythonChat>, public IAbstractChat
{
	...
	public:
		...
		static BYTE GetEmpire();
		void AppendChat(int iType, const char* c_szChat, BYTE bSpecialColorType = IAbstractChat::ESpecialColorType::CHAT_SPECIAL_COLOR_NORMAL, BYTE bEmpire = 0, const std::string& countryName = "") override;
#else
...
#if defined(__BL_MULTI_LANGUAGE_PREMIUM__)
		void AppendWhisper(int iType, const char * c_szName, const char * c_szChat, BYTE bEmpire = 0, const std::string& countryName = "");
#else
