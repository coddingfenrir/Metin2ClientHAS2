...
{
	// DEBUG: Loaded / Missing skill list
	static std::set<DWORD> loggedLoaded;
	static std::set<DWORD> loggedMissing;

	TSkillDataMap::iterator it = m_SkillDataMap.find(dwSkillIndex);

	if (m_SkillDataMap.end() == it)
	{
		// Missing skill ID only 1 kez loglansýn
		if (loggedMissing.find(dwSkillIndex) == loggedMissing.end())
		{
			TraceError("Missing skill: %u", dwSkillIndex);
			loggedMissing.insert(dwSkillIndex);
		}
		return FALSE;
	}

	// Loaded skill ID only 1 kez loglansýn
	if (loggedLoaded.find(dwSkillIndex) == loggedLoaded.end())
	{
		TraceError("Loaded skill: %u", dwSkillIndex);
		loggedLoaded.insert(dwSkillIndex);
	}
...
PyObject* skillGetSkillType(PyObject* poSelf, PyObject* poArgs)
{
	...
	// FIX: 0 veya negatif skill index gelirse -> boþ slot -> return 0
	if (iSkillIndex <= 0)
		return Py_BuildValue("i", 0);

	CPythonSkill::SSkillData* c_pSkillData = NULL;

	// FIX: Skill bulunamazsa exception yerine güvenli 0 dön
	if (!CPythonSkill::Instance().GetSkillData(iSkillIndex, &c_pSkillData))
		return Py_BuildValue("i", 0);
