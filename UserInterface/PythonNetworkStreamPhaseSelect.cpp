...
bool CPythonNetworkStream::SendSelectCharacterPacket(BYTE Index)
{
	...
	std::string country = LocaleService_GetLocaleName();
#if defined(__BL_MULTI_LANGUAGE_ULTIMATE__)
	if (CPythonSystem::Instance().GetAnonymousCountryMode())
		country = "eu";
#endif
	strncpy(SelectCharacterPacket.country, country.c_str(), sizeof(SelectCharacterPacket.country) - 1);
	SelectCharacterPacket.country[sizeof(SelectCharacterPacket.country) - 1] = '\0';
#endif
