...
PyObject* systemSetAnonymousCountryMode(PyObject* poSelf, PyObject* poArgs)
{
	bool bEnable;
	if (!PyTuple_GetBoolean(poArgs, 0, &bEnable))
		return Py_BuildException();

	CPythonSystem::Instance().SetAnonymousCountryMode(bEnable);
	return Py_BuildNone();
}

PyObject* systemGetAnonymousCountryMode(PyObject* poSelf, PyObject* poArgs)
{
	return Py_BuildValue("b", CPythonSystem::Instance().GetAnonymousCountryMode());
}

PyObject* systemSetShowCountryFlag(PyObject* poSelf, PyObject* poArgs)
{
	bool bEnable;
	if (!PyTuple_GetBoolean(poArgs, 0, &bEnable))
		return Py_BuildException();

	CPythonSystem::Instance().SetShowCountryFlag(bEnable);
	return Py_BuildNone();
}

PyObject* systemIsShowCountryFlag(PyObject* poSelf, PyObject* poArgs)
{
	return Py_BuildValue("b", CPythonSystem::Instance().IsShowCountryFlag());
}

PyObject* systemSetShowEmpireFlag(PyObject* poSelf, PyObject* poArgs)
{
	bool bEnable;
	if (!PyTuple_GetBoolean(poArgs, 0, &bEnable))
		return Py_BuildException();

	CPythonSystem::Instance().SetShowEmpireFlag(bEnable);
	return Py_BuildNone();
}

PyObject* systemIsShowEmpireFlag(PyObject* poSelf, PyObject* poArgs)
{
	return Py_BuildValue("b", CPythonSystem::Instance().IsShowEmpireFlag());
}

PyObject* systemAddChatFilterCountry(PyObject* poSelf, PyObject* poArgs)
{
	char* szCountry;
	if (!PyTuple_GetString(poArgs, 0, &szCountry))
		return Py_BuildException();

	CPythonSystem::Instance().AddChatFilterCountry(szCountry);
	return Py_BuildNone();
}

PyObject* systemRemoveChatFilterCountry(PyObject* poSelf, PyObject* poArgs)
{
	char* szCountry;
	if (!PyTuple_GetString(poArgs, 0, &szCountry))
		return Py_BuildException();

	CPythonSystem::Instance().RemoveChatFilterCountry(szCountry);
	return Py_BuildNone();
}

PyObject* systemIsChatFilterCountry(PyObject* poSelf, PyObject* poArgs)
{
	char* szCountry;
	if (!PyTuple_GetString(poArgs, 0, &szCountry))
		return Py_BuildException();

	return Py_BuildValue("b", CPythonSystem::Instance().IsChatFilterCountry(szCountry));
}

PyObject* systemAddChatFilterEmpire(PyObject* poSelf, PyObject* poArgs)
{
	BYTE bEmpire;
	if (!PyTuple_GetByte(poArgs, 0, &bEmpire))
		return Py_BuildException();

	CPythonSystem::Instance().AddChatFilterEmpire(bEmpire);
	return Py_BuildNone();
}

PyObject* systemRemoveChatFilterEmpire(PyObject* poSelf, PyObject* poArgs)
{
	BYTE bEmpire;
	if (!PyTuple_GetByte(poArgs, 0, &bEmpire))
		return Py_BuildException();

	CPythonSystem::Instance().RemoveChatFilterEmpire(bEmpire);
	return Py_BuildNone();
}

PyObject* systemIsChatFilterEmpire(PyObject* poSelf, PyObject* poArgs)
{
	BYTE bEmpire;
	if (!PyTuple_GetByte(poArgs, 0, &bEmpire))
		return Py_BuildException();

	return Py_BuildValue("b", CPythonSystem::Instance().IsChatFilterEmpire(bEmpire));
}
#endif
...
#if defined(__BL_MULTI_LANGUAGE_ULTIMATE__)
		{ "SetAnonymousCountryMode",	systemSetAnonymousCountryMode,	METH_VARARGS },
		{ "GetAnonymousCountryMode",	systemGetAnonymousCountryMode,	METH_VARARGS },

		{ "SetShowCountryFlag",			systemSetShowCountryFlag,		METH_VARARGS },
		{ "IsShowCountryFlag",			systemIsShowCountryFlag,		METH_VARARGS },

		{ "SetShowEmpireFlag",			systemSetShowEmpireFlag,		METH_VARARGS },
		{ "IsShowEmpireFlag",			systemIsShowEmpireFlag,			METH_VARARGS },

		{ "AddChatFilterCountry",		systemAddChatFilterCountry,		METH_VARARGS },
		{ "RemoveChatFilterCountry",	systemRemoveChatFilterCountry,	METH_VARARGS },
		{ "IsChatFilterCountry",		systemIsChatFilterCountry,		METH_VARARGS },

		{ "AddChatFilterEmpire",		systemAddChatFilterEmpire,		METH_VARARGS },
		{ "RemoveChatFilterEmpire",		systemRemoveChatFilterEmpire,	METH_VARARGS },
		{ "IsChatFilterEmpire",			systemIsChatFilterEmpire,		METH_VARARGS },
#endif
