...
PyObject * itemGetIconImageFileName(PyObject * poSelf, PyObject * poArgs)
{
	...
	// The icon image pointer is stored in CItemData (set when the item table is loaded).
	// CItemData::GetIconImage() returns a CGraphicSubImage* that wraps the loaded graphic resource.
	// CGraphicSubImage::GetFileName() returns the original filename used when the resource was created
	// (typically paths like "d:/ymir work/ui/items/..." or whatever your resource definitions contain).
...
int GetApplyPoint(int iApply)
{
	static const std::vector<TApplyInfo> aApplyInfo =
	{
		...
		{ POINT_BONUS_BOSS_DMG },
	#endif// APPLY_ATTBONUS_BOSS,119
	#ifdef ENABLE_BONUS_METIN
			{ POINT_BONUS_METIN_DMG },
	#endif// APPLY_ATTBONUS_BOSS,120
	};
