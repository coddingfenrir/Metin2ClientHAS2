...
PyObject * chatAppendWhisper(PyObject* poSelf, PyObject* poArgs)
{
	...
	{
		if (!PyTuple_GetByte(poArgs, 3, &bEmpire))
			return Py_BuildException();
	}
	...
	{
		char* szCountry;
		if (!PyTuple_GetString(poArgs, 4, &szCountry))
			return Py_BuildException();

		country = szCountry;
	}
	CPythonChat::Instance().AppendWhisper(iType, szName, szChat, bEmpire, country);
#else
...
#if defined(__BL_MULTI_LANGUAGE_PREMIUM__)
PyObject* chatGetCountry(PyObject* poSelf, PyObject* poArgs)
{
	const std::string& country = CPythonChat::GetCountry();
	return Py_BuildValue("s", country.c_str());
}

PyObject* chatGetEmpire(PyObject* poSelf, PyObject* poArgs)
{
	const BYTE empire = CPythonChat::GetEmpire();
	return Py_BuildValue("i", empire);
}
#endif
...
#if defined(__BL_MULTI_LANGUAGE_PREMIUM__)
		{ "GetCountry",				chatGetCountry,				METH_VARARGS },
		{ "GetEmpire",				chatGetEmpire,				METH_VARARGS },
#endif
