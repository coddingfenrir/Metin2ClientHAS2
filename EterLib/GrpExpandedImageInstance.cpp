...
bool CGraphicExpandedImageInstance::IsMouseOverIn(int mx, int my)
{
	if (IsEmpty())
		return false;

	const float sx = m_v2Position.x;
	const float ex = m_v2Position.x + GetScaledWidth();
	const float sy = m_v2Position.y;
	const float ey = m_v2Position.y + GetScaledHeight();

	return (mx >= sx && mx <= ex && my >= sy && my <= ey);
}

int CGraphicExpandedImageInstance::GetScaledWidth() const
{
	if (IsEmpty())
		return 0;
	return m_roImage->GetWidth() * m_v2Scale.x + 0.5f;
}

int CGraphicExpandedImageInstance::GetScaledHeight() const
{
	if (IsEmpty())
		return 0;
	return m_roImage->GetHeight() * m_v2Scale.y + 0.5f;
}
#endif
